...
1. Investigate model behavior on subsets of the data partitioned on set piece type.
2. Analyze model predictions on a team-by-team basis as a consistency check.
3. Build code infrastructure to handle full event tracking data. By the full event tracking data, we mean the paid version of the data set used in this project that tells you not only the starting and ending locations of the ball for each event, but also those locations for every player on the field. Perhaps the most important aspect to this is determining new features to "engineer" using the additional information of the paid data.
