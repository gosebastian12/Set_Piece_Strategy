...
def authentication_header_generator(
	...
	quickly generate an authentification string header to access the 
	Wyscout Events API. This API requires a Basic Authentification header
...
def raw_event_data(league_name: str) -> pd.DataFrame:
	...
	La Liga, Seria A, Bundesliga, European Championships, and World Cup),
	all the user has to specify is the league they want data for. By raw
	data, it is meant that it is the dat that can be obtained at the link
	...
	try:
		...
		# If the test fails, raise a `ValueError` and print the approriate
		# errror message.
